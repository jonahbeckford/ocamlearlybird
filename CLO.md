...
Timings come from GitHub Actions runners running dk <!--VER:dk-->2.4.3.20<!--/VER:dk-->
(`.github/workflows/measure-performance.yml`). Every figure below is the mean of
repeated runs of that workflow at one pin. Absolute times scale with core count and
...
| --- | --- | --- |
| Install dk1 | ~5 s | ~5 s |
| First run (fetch toolchain + build closure) | ~3 m 1 s ±6% | ~9 m 2 s ±8% |
| Warm re-run | ~7 s ±16% | ~18 s ±29% |
<!--/PERF:quicksetup-->
...
with dk <!--VER:dk-->2.4.3.20<!--/VER:dk--> and opam 2.5.2 (the `opam` figures include the switch create
and compiler install):

<!--PERF:comparison-->
| Step | dk Quick Setup | opam + dune |
| --- | --- | --- |
| Linux: fresh checkout (desktop) | ~3 m 1 s ±6% | ~4 m 32 s ±20% |
| Linux: fresh checkout (CI cache) | — | ~2 m 19 s ±10% |
| Linux: re-run the built binary | ~7 s ±16% | ~0.1 s ±14% |
| Linux: edit one file, rebuild | ~17 s ±10% | ~0.2 s ±15% |
| Windows: fresh checkout (desktop) | ~9 m 2 s ±8% | ~14 m 35 s ±11% |
| Windows: fresh checkout (CI cache) | — | ~8 m 18 s ±13% |
| Windows: re-run the built binary | ~18 s ±29% | ~1.1 s ±19% |
| Windows: edit one file, rebuild | ~44 s ±7% | ~1.3 s ±19% |
<!--/PERF:comparison-->
...
`dune build @install`. Neither figure includes
...
because the cache does not touch them.
...
| Step | Linux_x86_64 | Windows_x86_64 |
| --- | --- | --- |
| Edit one file, rebuild | ~17 s ±10% | ~44 s ±7% |
<!--/PERF:edit-->

Every figure is the mean of repeated runs of
`.github/workflows/measure-performance.yml` at one pin.
