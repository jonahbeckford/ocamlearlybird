...
| Step | Linux_x86_64 | Windows_x86_64 |
| --- | --- | --- |
| Fetch the prebuilt closure and run | ~3 m 20 s | ~3 m 33 s |
| Warm re-run | ~6 s | ~14 s |

For comparison, Quick Setup's first build on the same runners is ~3 m 34 s
(Linux) and ~10 m 25 s (Windows), and a from-scratch `opam switch create` +
`opam install` + `dune build` is ~5 m (Linux) and ~16 m (Windows).
